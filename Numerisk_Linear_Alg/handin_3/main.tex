...
Comparing the plots from (b), (c) and (f) it's apparent from the get-go that (c) is arguably the worst fit, since forcing a 4 degree polynomial through all the points seems make the polynomial swing/oscillate beteween some of the points. Comparatively to the other two, (b) and (f) seems to fit what we expect from the data, a steady increase in temeprature.\\
Between (b) and (f), the Author thinks (f) is a better fit since it fits all the data points while keeping the trend of a steady increase between the points, while (b) only accounts for the last three data points.
